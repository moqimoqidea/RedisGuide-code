...
class TestIdentityLock(unittest.TestCase):
    ...
    def test_lock_is_unique(self):
        self.assertTrue(self.lock.acquire(self.identity, self.timeout))

        self.assertFalse(self.lock.acquire(self.identity, self.timeout))

    def test_lock_release_when_identity_match(self):
        self.lock.acquire(self.identity, self.timeout)

        self.assertFalse(self.lock.release("wrong identity"))

        self.assertTrue(self.lock.release(self.identity))
