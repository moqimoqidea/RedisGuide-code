...
class TestLocation(unittest.TestCase):
    ...
    def test_pin(self):
        self.assertIsNone(self.location.get("peter"))

        self.location.pin("peter", self.peter["long"], self.peter["lati"])

        self.assertIsNotNone(self.location.get("peter"))
    ...
    def test_get(self):
        ...
        self.assertEqual(
            self.location.get("peter"), (self.peter["long"], self.peter["lati"])
        )
    ...
    def test_calculate_distance(self):
        ...
        self.assertGreater(self.location.calculate_distance("peter", "jack"), 0)
        self.assertLess(self.location.calculate_distance("peter", "jack"), 100)
    ...
    def test_find_nearby(self):
        ...
        self.assertEqual(len(self.location.find_nearby("peter", 50)), 0)
        self.assertEqual(len(self.location.find_nearby("peter", 60)), 1)
        self.assertEqual(len(self.location.find_nearby("peter", 70)), 2)
    ...
    def test_random_nearby(self):
        ...
        self.assertTrue(random_user == "jack" or random_user == "tom")

    def test_cached_find_nearby(self):
        self.location.pin("peter", self.peter["long"], self.peter["lati"])
        self.location.pin("jack", self.jack["long"], self.jack["lati"])

        self.assertEqual(len(self.location.find_nearby("peter", 70)), 1)
        ...
        print("Testing cache:")
        print(
            "Sleep {0} seconds, wait for the cache to expire.".format(RADIUS_CACHE_TIME)
        )
