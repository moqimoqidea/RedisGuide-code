...
class Location:
    ...
    def find_nearby(self, user, radius=1):
        ...
        if (last_cache_time is not None) and (
            float(last_cache_time) + RADIUS_CACHE_TIME < current_time
        ):
            # 缓存已过期，删除它
        ...
        result = self.client.georadiusbymember(
            self.key, user, radius, unit="km"
        )  # 获取附近的所有用户
        all_nearby_users = filter(
            lambda other_user: other_user != user, result
        )  # 移除结果中的 user 自身
        if all_nearby_users != []:
            # 缓存查找结果
